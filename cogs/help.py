...
class vein9(commands.Cog, name='Help'):
    ...
    async def help(self, ctx, *, entity=None):
        ...
        if not entity:
            ...
            embed1.add_field(
                name = "Music commands",  value=f"``help_music`` or ``hm`` for this part :)")
            embed1.add_field(
                name="Handy commands", value=f'``ping`` ``invite`` ``lenny`` ``f`` ``hi`` ``flip`` ``calc`` ``anime`` ``manga`` ``wuxia`` ``hsearch``', inline=False)
            embed1.add_field(
